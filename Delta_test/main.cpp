...
{
    const unsigned int BUFFER_SIZE = 1024*128;
...

    {
        ...
        std::cout<< file_name <<" is copied, file size is " <<(file_size / 1024) <<
                ...
                "Speed: " << (file_size / (1024 * elapsed_time))<<std::endl;
    }
    return;
}
...
/*    std::ofstream test_file("first.dat", std::ios_base::binary);
    ...
    while(!files.empty())
    {
        tPool.add(CopyFile, files.front(), dist, &ex_list, max_rate);
        files.pop_front();
    }
